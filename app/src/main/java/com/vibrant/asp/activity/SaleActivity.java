...
import com.vibrant.asp.adapter.ProductsForSaleAdapter;
...
public class SaleActivity extends AppCompatActivity {
    ...
    ImageView ivBack;
    ProgressDialog pd;
    List<ProductsForSale> forSaleList  = new ArrayList<>();
    RecyclerView recyclerView;
    ProductsForSaleAdapter mAdapter;
    ...
    private void init() {
        ...
        tvHeader.setText(getString(R.string.sale_orders));
        ...
        if (isInternetConnected(getApplicationContext())) {
          //  getSaleOrder();
            ...
            showToast(SaleActivity.this, getResources().getString(R.string.check_network));
        }
    }

    private void getSaleOrder() {
        String url = Cons.GET_FOR_SALE;
        pd = ProgressDialog.show(SaleActivity.this, "Please Wait...");
        JSONObject jsonObject = new JSONObject();
        try {
            String mRenterId = getPreference(SaleActivity.this, "renterId");
            if (mRenterId != null) {
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.POST, url, jsonObject, new Response.Listener<JSONObject>() {
                ...
                try {
                    ...
                    if (jsonArray.length() > 0) {
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ProductsForSale getOrdersForRenter = new ProductsForSale();
                           // getOrdersForRenter.setRenteeName(jsonArray.getJSONObject(i).getString("RenteeName"));
                            forSaleList.add(getOrdersForRenter);
                        }
                        if (forSaleList.size() > 0) {
                            tvNoRecord.setVisibility(View.GONE);
                            mAdapter = new ProductsForSaleAdapter(SaleActivity.this, forSaleList);
                            RecyclerView.LayoutManager mLayoutManager = new LinearLayoutManager(SaleActivity.this);
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                pd.dismiss();
                showToast(SaleActivity.this, "Something went wrong");
            }
        }) {
            @Override
